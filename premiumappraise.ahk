...
global delayFirst := 100
global delaySecond := 100
global delayThird := 400
global movePause := 300
global tolerance := 2
global roamRadius := 2
...
global firstClickDefined := true
global clickX := 693
global clickY := 636
global secondClickDefined := true
global secondClickX := 1028
global secondClickY := 847
...
colors["Poisoned"] := "0x674991"
colors["Lunar"] := "0xBDA9FF"
...
Gui, Add, Text,, Press F3 to select the mutation area and press F3 again to save it
Gui, Add, Text, vClickLabel xm, First click: 693,636 | Second click: 1028,847
Gui, Add, Text,, Press F1 to save the "appraise" click and F2 to save the "confirm" click

Gui, Add, GroupBox, xm y+10 w280 h220, Mutations to scan for (can select multiple)
...
Gui, Add, Checkbox, xp yp+25 vColorPoisoned, Poisoned
Gui, Add, Checkbox, xp yp+25 vColorLunar, Lunar
...
Gui, Show,, premium appraiser by rando
UpdateAreaLabel()
UpdateClickLabel()
return
...
StartLoop:
    ...
    if (!firstClickDefined) {
        MsgBox, 48, First Click Not Set, Please press F1 to save the first click location before starting.
        return
    }

    if (!secondClickDefined) {
        MsgBox, 48, Second Click Not Set, Please press F2 to save the second click location before starting.
        return
    }

    if (!areaDefined) {
        MsgBox, 48, Area Not Set, Please select an area with F3 before starting.
        return
    }
...
RunCycle:
    ...
    Click
    Sleep, delayFirst
    MoveMouseWithRoam(secondX, secondY, movePause, roamRadius)
    Click
    Sleep, delaySecond
...
F3::
    if (!selectionActive) {
        ...
        ToolTip, % "Drag with the left mouse button to set the area, then press F3 to save.", 0, 0
        ...
        if (selectionPhase != "preview") {
            ToolTip, Drag with the mouse before pressing F3 again., 0, 0
            SetTimer, ClearToolTip, -1500
            return
        }
        selectionActive := false
...
F1::
    ...
    firstClickDefined := true
    UpdateClickLabel()
    ToolTip, % "Saved first click at (" clickX ", " clickY ")"
    SetTimer, ClearToolTip, -1200
return
...
F2::
    if (selectionActive)
        return
    MouseGetPos, secondClickX, secondClickY
    secondClickDefined := true
    UpdateClickLabel()
    ToolTip, % "Saved second click at (" secondClickX ", " secondClickY ")"
    SetTimer, ClearToolTip, -1200
return
...
UpdateClickLabel() {
    global firstClickDefined, clickX, clickY, secondClickDefined, secondClickX, secondClickY
    firstText := firstClickDefined ? clickX "," clickY : "not set"
    secondText := secondClickDefined ? secondClickX "," secondClickY : "not set"
    text := "First click: " firstText " | Second click: " secondText
